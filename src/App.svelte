<script>
  ...
  const onKeyPress = e => {
    if (e.charCode === 13 && value.length > 0) addTodo();
  };

  function addTodo() {
		todos = [...todos, { id: todoIndex, name: value, completed: false}];
    todoIndex += 1;
    value = '';
	}
  ...
  $: filteredList = todos.filter(item => {
    if (filteredBy) {
      let completed = filteredBy === 'completed';
      ...
      {#if todos.length > 0}
        <footer class="footer" >
          ...
          <ul class="filters">
            ...
            <li>
              <a href="#/active" class:selected={filteredBy === 'active'} on:click={showActive}>Active</a>
            </li>
            <li>
              <a href="#/completed" class:selected={filteredBy === 'completed'} on:click={showCompleted}>Completed</a>
            </li>
          </ul>
